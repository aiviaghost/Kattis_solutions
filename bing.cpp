...
auto contains(Node* trie, const char* word, bool found) -> void {
    if (*word) {
        if (!trie->children[*word - 'a']) {
            return;
        }
        contains(trie->children[*word - 'a'], word + 1, found);
    }
    else {
        found = true;
        ...
        for (int i = 0; i < 26; i++) {
            if (trie->children[i]) {
                contains(trie->children[i], word, found);
            }
        }
    }
}
...
auto main() -> int {
    ...
    while (n--) {
        ...
        contains(trie, word.c_str(), false);
