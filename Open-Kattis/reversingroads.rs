...
unsafe fn solve(input: &str) {
    ...
    let mut t = 1;
    while !scan.eof() {
        let (m, n) = (scan.u(), scan.u());
        let mut all_edges: Vec<(usize, usize)> = Vec::new();
        let mut adj: Vec<Vec<usize>> = vec![Vec::new(); m];
        for i in 0..n {
            let (a, b) = (scan.u(), scan.u());
            adj[a].push(b);
            all_edges.push((a, b));
        }

        let (mut sccs, _, _) = scc(&adj);
        if sccs.len() == 1 {
            println!("Case {}: valid", t);
        } else {
            let mut found = false;
            for i in 0..all_edges.len() {
                let mut new_adj: Vec<Vec<usize>> = vec![Vec::new(); m];
                for (j, (a, b)) in all_edges.iter().enumerate() {
                    if i == j {
                        new_adj[*b].push(*a);
                    } else {
                        new_adj[*a].push(*b);
                ...
                (sccs, _, _) = scc(&new_adj);
                if sccs.len() == 1 {
                    println!("Case {}: {} {}", t, all_edges[i].0, all_edges[i].1);
                    found = true;
                    break;
                }
            }
            if !found {
                println!("Case {}: invalid", t);
            }
        }
