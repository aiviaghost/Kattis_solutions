...
/// * The SCCs in topoligcal order
/// * The SCC each node is a part of
/// * A boolean representing "The graph is a DAG"
///
/// If the graph is a DAG, then each SCC will only have one element and you can
/// get the normal topological sort from the first element of the return value:
/// ```
/// scc.into_iter().flatten().vec()
/// ```
///
/// You can create the condensation graph afterwards by going through the
/// adjacency list and just adding the edges where the nodes are part of
/// different SCCs.
pub fn scc(adj: &[Vec<usize>]) -> (Vec<Vec<usize>>, Vec<usize>, bool) {
    let mut scc: Vec<Vec<usize>> = vec![];
    let mut map = vec![0; adj.len()];

    let mut stack = vec![];
    {
        let mut visited = vec![false; adj.len()];
        let mut dfs = RecursiveFunction1::new(|f, node: usize| {
            if !visited[node] {
                visited[node] = true;
    ...
    let mut adj_rev = vec![vec![]; adj.len()];
    for (to, adj) in adj.iter().enumerate() {
        for &from in adj {
            adj_rev[from].push(to);
        }
    }
    {
        let mut visited = vec![false; adj.len()];
        while let Some(node) = stack.pop() {
            if !visited[node] {
                ...
                let mut dfs = RecursiveFunction1::new(|f, node: usize| {
                    if !visited[node] {
                        visited[node] = true;
                        scc.last_mut().unwrap().push(node);
                        map[node] = scc.len() - 1;
                        for &other in &adj_rev[node] {
                            f.call(other);
                        }
                    }
                });
    ...
    let is_dag = scc.len() == map.len();
    (scc, map, is_dag)
}

unsafe fn solve(input: &str) {
    let mut scan = input.scanner();

    let T = scan.u();

    for _ in 0..T {
        let (n, m) = (scan.u(), scan.u());
        let mut adj: Vec<Vec<usize>> = vec![Vec::new(); n];
        let mut all_edges: Vec<(usize, usize)> = Vec::new();
        for _ in 0..m {
            let (s1, s2) = (scan.u(), scan.u());
            adj[s1 - 1].push(s2 - 1);
            all_edges.push((s1 - 1, s2 - 1));
        }
        let (sccs, map, _) = scc(&adj);
        let mut in_deg_total = vec![0; sccs.len()];
        let mut out_deg_total = vec![0; sccs.len()];
        for (a, b) in all_edges.iter() {
            if map[*a] != map[*b] {
                in_deg_total[map[*b]] = 1.min(in_deg_total[map[*b]] + 1);
                out_deg_total[map[*a]] = 1.min(out_deg_total[map[*a]] + 1);
            }
        }
        ...
        if sccs.len() == 1 {
            ...
            outln!(
                "{}",
                (sccs.len() - in_deg_total.iter().sum::<usize>())
                    .max(sccs.len() - out_deg_total.iter().sum::<usize>())
            )
        }
    }
}
...
pub fn main() {
    ...
    unsafe {
        ...
        //     None,
        //     io::stdin().lock().lines().flat_map(|l| {
        //         storage.push(l.unwrap());
        //         (*(storage.last().unwrap().as_str() as *const str)).split_whitespace()
        //     }),
        // );
        // solve(&mut scan);
    }
...
pub mod input {
    ...
    impl<'a, I: Iterator<Item = &'a str>> Scanner<'a, I> {
        ...
        /// collect into a Vec.
        pub fn vec<T: FromStr>(&mut self) -> Vec<T> {
            self.collect()
        }
...
pub mod math {
    ...
    pub fn sieve(max_value: usize) -> Vec<usize> {
        let mut v = vec![0; max_value + 1];
        for x in 2..=max_value {
            if v[x] == 0 {
                for u in (2 * x..).step_by(x).take_while(|&u| u <= max_value) {
                    v[u] = x;
                }
            }
        }
...
pub mod recursive_function {
    ...
    recursive_function!(RecursiveFunction1, Callable1, (Arg1 arg1,));
