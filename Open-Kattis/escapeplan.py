...
while True:
	...
	stdout.write(f"Scenario {t}\n")
	for time in (5, 10, 20):
		edges = [(i[0], m + j[0], 1) for i, j in product(*map(enumerate, (robots, holes))) if dist(*i[1], *j[1]) <= time * 10]
		D = Dinitz(n + m + 2)
		for edge in edges:
			D.add_edge(*edge)
		for i in range(m):
			D.add_edge(n + m, i, 1)
		for i in range(n):
			D.add_edge(m + i, n + m + 1, 1)
		stdout.write(f"In {time} seconds {D.max_flow(n + m, n + m + 1)} robot(s) can escape\n")
	stdout.write("\n")
	t += 1
