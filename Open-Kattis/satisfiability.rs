    ...
    I: Iterator<Item = &'a str>,
{
    let nm = scan
        ...
        .collect::<Vec<usize>>();
    let (n, m) = (nm[0], nm[1]);
    let mut sat = vec![vec![0; 2 * n]; m];
    for i in 0..m {
        for s in scan.s().split(" v ") {
            if s.chars().next().unwrap() == '~' {
                let j = s[2..].parse::<usize>().unwrap() - 1;
                sat[i][n + j] = 1;
            } else {
                let j = s[1..].parse::<usize>().unwrap() - 1;
                sat[i][j] = 1;
            }
        }
    }
    ...
    'outer: for mask in 0..1 << n {
        ...
        for i in 0..m {
            ...
            for j in 0..n {
                let bit = (mask >> j) & 1;
                if (bit == 0 && sat[i][n + j] == 1) || (bit == 1 && sat[i][j] == 1) {
                    ok = true;
                    break;
                }
            }
...
pub fn main() {
    ...
    unsafe {
        ...
        // use custom scanner instead because this one splits on whitespace!!!!!
        let mut scan = Scanner::custom(None, input.split("\n"));
        let T = scan.u();
        for _ in 0..T {
            solve(&mut scan);
        }
